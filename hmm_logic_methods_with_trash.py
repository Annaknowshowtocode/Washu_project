...
def train_model_prepared(
    ...
    for split_num, per_length_data in per_kfold_per_length_data.items():
        ...
        # Перемешиваем тренировочные данные и веса ОДИНАКОВО
...
def get_number_of_prepared_emission_positions(model_training_params: ModelTrainingParams) -> int:
    states_per_group = model_training_params.states_per_group
    number_of_groups = model_training_params.groups
    model_complexity = model_training_params.model_complexity
    anchor_states = model_training_params.anchor_states

    if isinstance(states_per_group, list):
        group_states_count = sum(states_per_group)
    else:
        group_states_count = states_per_group * number_of_groups

    number_of_joiners = 2 * model_complexity if anchor_states else 0

    return group_states_count + number_of_joiners
...
def prepare_multiple_models(
    experiment_params: ExperimentParams,
    custom_models_id: str = "root",
    add_regular_models: bool = True,
    add_trash_model: bool = True):
    ...
    for allele_name in alleles_to_use:
        ...
        if add_regular_models:
            for run_num in range(total_runs):
                ...
                for split_num in range(data_scenario_params.splits_to_read):
                    ...
                    prepared_model.name = (
                        f'{run_index}_run-{current_params.get_model_common_names()}'
                        f'_model-{target_allele_name}-{split_num}'
                    )
        ...
        # trash-модель
        if add_trash_model:
            trash_run_index = f"{custom_models_id}[trash]"
            per_allele_per_run_per_split_prepared_models[allele_name][trash_run_index] = dict()

            trash_params = deepcopy(model_training_params)

            trash_positions_count = get_number_of_prepared_emission_positions(trash_params)
            emission_matrix_trash = get_frequencies(trash_positions_count)

            for split_num in range(data_scenario_params.splits_to_read):
                trash_model = build_model_based_on_params(
                    model_training_params=trash_params,
                    amino_acids='ACDEFGHIKLMNPQRSTVWY',
                    prepared_emission_matrix=emission_matrix_trash,
                )

                trash_model.name = (
                    f'{trash_run_index}_trash-model-{target_allele_name}-{split_num}'
                )

                per_allele_per_run_per_split_prepared_models[allele_name][trash_run_index][split_num] = trash_model
...
def train_multiple_models(prepared_models,
    ...
    print(f"Сюда сохраняются модели {path_to_save_models}")
...
def _is_trash_run_index(run_index: str) -> bool:
    return str(run_index).endswith("[trash]")
...
def reorder_models_by_score_and_flatten_to_by_name_list(
    histories,
    models,
    experiment_params: ExperimentParams,
    subfolder_to_safe_result: str,
    ...
    first_run = list(models[first_allele].keys())[0]
    SPLITS = list(models[first_allele][first_run])

    per_name_new_models = dict()
    per_name_histories = dict()

    per_name_trash_models = dict()
    per_name_trash_histories = dict()
    ...
    for split_index in SPLITS:
        ...
        for allele_name in ALLELES:
            all_runs = list(models[allele_name].keys())

            regular_runs = [
                run_index for run_index in all_runs
                if not _is_trash_run_index(run_index)
            ]

            trash_runs = [
                run_index for run_index in all_runs
                if _is_trash_run_index(run_index)
            ]

            # ранжируем только обычные модели
            if regular_runs:
                df = pd.DataFrame({
                    "model": regular_runs,
                    "log_probability": [
                        histories[allele_name][run_index][split_index].log_probabilities[-1]
                        for run_index in regular_runs
                    ],
                })
                ...
                for new_index, model_run_index in zip(sorted(regular_runs), df.model):
            ...
            # trash-модели сохраняем отдельно, без участия в ranking
            for trash_run_index in trash_runs:
                trash_model = models[allele_name][trash_run_index][split_index]
                trash_history = histories[allele_name][trash_run_index][split_index]

                trash_name = trash_model.name
                per_name_trash_models[trash_name] = trash_model
                per_name_trash_histories[trash_name] = trash_history

    # сохраняем обычные модели
    for name, model in per_name_new_models.items():
        history = per_name_histories[name]
        save_model(path_to_save_models, model, history)

    # сохраняем trash-модели
    for name, model in per_name_trash_models.items():
        history = per_name_trash_histories[name]
        save_model(path_to_save_models, model, history)
...
def get_median_scores_for_top_models(per_name_models, allele_data_df, models_fraction=0.7):
    ...
    for model_index in range(number_of_models_to_estimate):
        ...
        scores[:, model_index] = [selected_model.log_probability(peptide) / len(peptide) for peptide in
                                  allele_data_df.peptide]  #тут пептиды
...
def reassign_data_to_splitted_models_according_to_strategy(
    per_name_models1,
    per_name_models2,
    per_name_trash_models,
    per_allele_data_df,
    experiment_params: ExperimentParams,
    trash_margin: float = 0.0):
    ...
    if data_split_strategy == "best_model":
        ...
        for allele_name in ALLELES:
            ...
            df["score1"] = get_median_scores_for_top_models(
                per_name_models=per_name_models1,
                allele_data_df=df,
                models_fraction=model_training_params.enrichment_split_decision_models_fraction,
            )

            df["score2"] = get_median_scores_for_top_models(
                per_name_models=per_name_models2,
                allele_data_df=df,
                models_fraction=model_training_params.enrichment_split_decision_models_fraction,
            )
    ...
    for allele_name in ALLELES:
        ...
        best_real_score = df[["score1", "score2"]].max(axis=1)

        trash_mask = df["score_trash"] > best_real_score + trash_margin

        model1_mask = (~trash_mask) & (df["score1"] >= df["score2"])
        model2_mask = (~trash_mask) & (df["score2"] > df["score1"])

        df_trash = df[trash_mask].copy()
        df1 = df[model1_mask].copy()
        df2 = df[model2_mask].copy()

        print(
            f"{allele_name}: "
            f"model1={len(df1)}, "
            f"model2={len(df2)}, "
            f"trash={len(df_trash)}, "
            f"total={len(df)}"
        )

        per_allele_first_df[allele_name] = df1
        per_allele_second_df[allele_name] = df2
        per_allele_trash_df[allele_name] = df_trash

    new_train_data1 = split_to_dicts(
        per_allele_first_df,
        ALLELES,
        TARGET_LENGTHS,
        TOTAL_SPLITS,
    ...
    new_train_data2 = split_to_dicts(
        per_allele_second_df,
        ALLELES,
        TARGET_LENGTHS,
        TOTAL_SPLITS,
    )

    new_train_data_trash = split_to_dicts(
        per_allele_trash_df,
        ALLELES,
        TARGET_LENGTHS,
        TOTAL_SPLITS,
    ...
    new_train_data_weights1 = calculate_weights_based_on_length_counts(
        new_train_data1,
        experiment_params=experiment_params,
    )

    new_train_data_weights2 = calculate_weights_based_on_length_counts(
        new_train_data2,
        experiment_params=experiment_params,
    )

    new_train_data_weights_trash = calculate_weights_based_on_length_counts(
        new_train_data_trash,
        experiment_params=experiment_params,
    )
    ...
    return (
        ...
        new_train_data2,
        new_train_data_trash,
        new_train_data_weights1,
        new_train_data_weights2,
        new_train_data_weights_trash,
    )
...
def get_average_prob_matrix_for_models(per_name_models, fraction_of_models=0.7):
    print("getting the average model matrix")
    model_names = list(per_name_models.keys())
    observed_alleles = list()
    model_models_counters = defaultdict(int)  # count models for each allele
    model_allele_to_matrix = dict()
...
def nested_train_data_dict():
    """
    Структура:
    train_data[allele_name][kfold][peptide_len] = [peptide1, peptide2, ...]
    """
    return defaultdict(lambda: defaultdict(lambda: defaultdict(list)))


def score_peptide_by_model(model, peptide):
    """
    Нормализованный score пептида по модели.
    """
    if peptide is None or len(peptide) == 0:
        return -np.inf

    try:
        return model.log_probability(peptide) / len(peptide)
    except Exception as e:
        print(f"Could not score peptide {peptide} by model {getattr(model, 'name', 'unknown')}: {e}")
        return -np.inf


def get_best_score_for_peptide(per_name_models, peptide):
    """
    Ищем лучшую модель из переданного словаря
    """
    best_score = -np.inf
    best_model_name = None

    if per_name_models is None:
        return best_score, best_model_name

    for model_name, model in per_name_models.items():
        score = score_peptide_by_model(model, peptide)
...
def make_single_split_and_enrich(per_name_models,
    ...
    per_allele_per_run_per_split_prepared_models1 = prepare_multiple_models(
        experiment_params,
        custom_models_id=f"{iteration_name}1",
        add_trash_model=False,
    )

    per_allele_per_run_per_split_prepared_models2 = prepare_multiple_models(
        experiment_params,
    ...
    (
        per_name_models1,
        per_name_histories1,
        per_name_trash_models1,
        per_name_trash_histories1,
        _,
    ...
    (
        per_name_models2,
        per_name_histories2,
        per_name_trash_models2,
        per_name_trash_histories2,
        _,
    ) = reorder_models_by_score_and_flatten_to_by_name_list(
    ...
    # bootstrap trash model before enrichment
    train_data_weights = calculate_weights_based_on_length_counts(
        train_data,
        experiment_params=experiment_params,
    )

    prepared_trash_models = prepare_multiple_models(
        experiment_params,
        custom_models_id=f"{iteration_name}trash_bootstrap",
        add_regular_models=False,
        add_trash_model=True,
    )

    trash_models_raw, trash_histories_raw = train_multiple_models(
        prepared_models=prepared_trash_models,
        train_data=train_data,
        test_data=test_data,
        train_data_weights=train_data_weights,
        experiment_params=experiment_params,
        subfolder_to_safe_result=f"{iteration_name}trash_bootstrap",
    )
    ...
    (
        ...
        histories=trash_histories_raw,
        models=trash_models_raw,
        experiment_params=experiment_params,
        subfolder_to_safe_result=f"{iteration_name}trash",
    )
    ...
    for i in range(enrichment_steps_count):
        ...
        (
            ...
            new_train_data_weights2,
            new_train_data_weights_trash,
        ) = reassign_data_to_splitted_models_according_to_strategy(
            per_name_models1=per_name_models1,
            per_name_models2=per_name_models2,
            per_name_trash_models=per_name_trash_models,
            per_allele_data_df=per_allele_data_df,
            experiment_params=experiment_params,
        )


        train_data1 = new_train_data1
        train_data2 = new_train_data2
        train_data_weights1 = new_train_data_weights1
        train_data_weights2 = new_train_data_weights2

        # обучение trash-модели
        prepared_trash_models = prepare_multiple_models(
            experiment_params,
            custom_models_id=f"{iteration_name}trash_enrich_{i}",
            add_regular_models=False,
            add_trash_model=True,
        )
        ...
        trash_models_raw, trash_histories_raw = train_multiple_models(
            prepared_models=prepared_trash_models,
            train_data=new_train_data_trash,
            test_data=test_data,
            train_data_weights=new_train_data_weights_trash,
            experiment_params=experiment_params,
            subfolder_to_safe_result=f"{iteration_name}trash_enrich_{i}",
        )
        ...
        per_allele_per_run_per_split_prepared_models1 = prepare_multiple_models(
            experiment_params,
            custom_models_id=f"{iteration_name}1_enrich_{i}",
            add_regular_models=True,
            add_trash_model=False,
        )

        per_allele_per_run_per_split_prepared_models2 = prepare_multiple_models(
            experiment_params,
            custom_models_id=f"{iteration_name}2_enrich_{i}",
            add_regular_models=True,
            add_trash_model=False,
        )
        ...
        per_allele_per_run_per_split_prepared_models1, \
            ...
            train_data_weights=train_data_weights1,
            experiment_params=experiment_params,
            subfolder_to_safe_result=f"{iteration_name}1_enrich_{i}",
        )
        ...
        per_allele_per_run_per_split_prepared_models2, \
            ...
            train_data_weights=train_data_weights2,
            experiment_params=experiment_params,
            subfolder_to_safe_result=f"{iteration_name}2_enrich_{i}",
        )
    ...
    with open('out.txt', 'w') as f:
        ...
        while iteration_deque:
            ...
            if (len(iteration_deque) == 0) or (len(iteration) < len(iteration_deque[0])):  # we finished the whole layer
                ...
                to_plot, fig = plot_split_diagnostics(
