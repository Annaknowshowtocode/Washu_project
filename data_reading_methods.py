...
def balance_peptides_preserve_length_distribution(per_allele_dict, alleles, seed=42):
    ...
    for s in splits:
        ...
        # 1) целевое распределение длин: по объединению всех аллелей
        ...
        # 4) семплируем по длинам для каждого аллеля
        for a in alleles:
            for L in lengths:
...
def collapse_alleles_to_one(per_allele_dict, dummy_allele_name="Dummy_all_as_one"):
    """
...
def get_train_test_data(
    ...
    if isinstance(data_params, PreprocessedIEDBDataParams):
        ...
        if collapse_to_dummy:
            ...
            if clean:
                non_clean_peptides_path = "/Users/annaklimova/Desktop/Washu_project/results/model_score_tables_IEDB_DRB1_0101_0301_0401/plot_df.csv"
            ...
            if clean:
                clean_peptides_path = "/Users/annaklimova/Desktop/Washu_project/clean_peptides_by_cell_type.csv"
...
#                     per_allele_per_split_per_length_new_data[allele_name][split_num][current_length] = \
#                     per_allele_per_split_per_length_data[allele_name][split_num][current_length]
#
...
def remove_unused_lengths(per_allele_per_split_per_length_data, experiment_params: ExperimentParams):
    ...
    ALLELES = sorted(per_allele_per_split_per_length_data.keys())
    print("ALLELES FROM DATA:", ALLELES)

    TARGET_LENGTHS = model_training_params.lengths_to_use
    print("TARGET_LENGTHS:", TARGET_LENGTHS)

    if not TARGET_LENGTHS:
        print("lengths_to_use is empty -> return input data unchanged")
        return per_allele_per_split_per_length_data
