...
from peptides_utils import defineClass, defineOrganism, split_to_dicts, join_dicts
...
def read_data(data_path, alleles_to_read, dataset_type, data_type, do_not_parse_alleles=False):
    ...
    for allele_name in alleles_to_read:
        ...
        for split_num in n_splits:
            ...
            for length in lengths:
                per_allele_per_kfold_per_length_peptides[allele_name][split_num][length] = total_df[(total_df['length'] == length) &
                                                                                                    (total_df['split'] == split_num)]['peptide'].unique()
    return per_allele_per_kfold_per_length_peptides



def get_train_test_data(experiment_params: ExperimentParams):
    """
    Read train/test data for preprocessed IEDB scenario.

    Returns:
        per_allele_per_kfold_per_length_binders_train
        per_allele_per_kfold_per_length_binders_test
        additional_return (list with per_allele_df)
    """
    ...
    # --- safety check: only IEDB allowed ---
    assert isinstance(
        data_params, PreprocessedIEDBDataParams
    ), "Only PreprocessedIEDBDataParams is supported"
    ...
    per_allele_per_kfold_per_length_binders_train = read_data(
        DATA_PATH, ALLELES, dataset_type="train", data_type="binders"
    )
    per_allele_per_kfold_per_length_binders_test = read_data(
        DATA_PATH, ALLELES, dataset_type="test", data_type="binders"
    )

    # --- sanity check: enough splits ---
    sample_allele = list(per_allele_per_kfold_per_length_binders_train.keys())[0]
    assert (
        len(per_allele_per_kfold_per_length_binders_train[sample_allele])
        >= TOTAL_SPLITS
    ), "Not enough splits in the data"

    # --- join data into per-allele dataframes ---
    per_allele_df = join_dicts(per_allele_per_kfold_per_length_binders_train)

    # add allele column explicitly
    for allele_name in ALLELES:
        per_allele_df[allele_name]["allele"] = allele_name

    additional_return = [per_allele_df]
...
def merge_per_allele_sets_into_combination_sets(per_allele_per_kfold_per_length_binders_train,
                                                per_allele_per_kfold_per_length_binders_test,
                                                combination_size, selected_alelles, total_splits, list_of_transform_dicts=None, position_component=False):
    f"""
    ...
    for combination in itertools.combinations(OLD_ALLELES, combination_size):
        ...
        for split_num in range(TOTAL_SPLITS):
            ...
            for target_length in intersected_lengths:
                ...
                for old_allele in combination:
                    single_allele_data_array_train = per_allele_per_kfold_per_length_binders_train[old_allele][split_num][target_length]
                    if list_of_transform_dicts:
                        single_allele_data_array_train = np.array([np.array([tuple(transform_dict[aa] for transform_dict in list_of_transform_dicts) for aa in peptide]) for peptide in single_allele_data_array_train])
                    if position_component:
                         single_allele_data_array_train = np.array([np.array([(aa, i) for i,aa in enumerate(peptide)]) for peptide in single_allele_data_array_train])
                        

                    if split_num == 0:
                        print(f"{old_allele} - {len(single_allele_data_array_train)}", end= " ")
                    data_arrays_train.append(single_allele_data_array_train)
                    single_allele_data_array_test = per_allele_per_kfold_per_length_binders_test[old_allele][split_num][target_length]
                    if list_of_transform_dicts:
                        single_allele_data_array_test = np.array([np.array([tuple(transform_dict[aa] for transform_dict in list_of_transform_dicts) for aa in peptide]) for peptide in single_allele_data_array_test])
                    if position_component:
                         single_allele_data_array_train = np.array([np.array([(aa, i) for i,aa in enumerate(peptide)]) for peptide in single_allele_data_array_test])
                ...
                per_allele_per_kfold_per_length_binders_train_new[allele_name][split_num][target_length] = np.concatenate(data_arrays_train)
                per_allele_per_kfold_per_length_binders_test_new[allele_name][split_num][target_length] = np.concatenate(data_arrays_test)
...
def calculate_weights_based_on_length_counts(original_peptide_data, experiment_params: ExperimentParams, verbose=False):
    f"""
    uses {extract_count_df}
    Just calculate portions of given length inside dataset if  {experiment_params.model_training_params.use_weights} is True, 
    else assign 1 to each sequence
    :param original_peptide_data: original training peptide data (should contain only used lengths
    :param experiment_params:  experiment params
    :return:
    """
...
def remove_unused_lengths(per_allele_per_split_per_length_data, experiment_params: ExperimentParams):
    ...
    ALLELES = sorted(model_training_params.alleles_to_use)
    TARGET_LENGTHS = model_training_params.lengths_to_use
    TOTAL_SPLITS = data_scenario_params.splits_to_read
    per_allele_per_split_per_length_new_data =  {allele_name:
                                                     {split_num: {
                                                         current_length: np.empty(0, dtype=str)
    ...
    for allele_name in ALLELES:
        for split_num in  range(TOTAL_SPLITS):
            for current_length in TARGET_LENGTHS:
                if current_length in per_allele_per_split_per_length_data[allele_name][split_num]:
                    per_allele_per_split_per_length_new_data[allele_name][split_num][current_length] = per_allele_per_split_per_length_data[allele_name][split_num][current_length]
