...
from pomegranate.distributions import (
    DiscreteDistributionAnchor,
    DiscreteDistributionCycle,
)
from pomegranate.io import DataGenerator, SequenceGenerator, BatchedDataGenerator
...
def train_model_prepared(
    ...
    for split_num, per_length_data in per_kfold_per_length_data.items():
        ...
        #print("train size {}".format(len(binders_array)), end=" ")

        # shuffle train data and weights in unison
...
#             batch_example = next(sequence_generator.batches())
#             print("Example of batch", batch_example)
#             sequence_generator.reset()
#         else:
#             sequence_generator = sample_X
#         print("Use pseudocount is", use_pseudocount)
...
def train_model(allele_name, 
    ...
    for split_num, per_length_data in per_kfold_per_length_data.items():
        ...
        print("split {}".format(split_num))
        print("train size {}".format(len(binders_array)))
        # total_length = len(binders_array)
        #  weights = [1 - per_length_counts[len(peptide)]/total_length for peptide in binders_array]
        # print("weights for lengths {}".format({key: 1 - value/total_length for key, value in per_length_counts.items()}))
        print("Shuffling for fairness ")
 ...
 \
        ...
        if prepared_models is None:
            ...
            model, history = HiddenMarkovModel.from_samples(distribution=DiscreteDistribution,
                                                            ...
                                                            emission_pseudocount=emission_pseudocount,
                                                            # init='random' # 'first-k', 'random', 'kmeans++', or 'kmeans||' Only for non-discrete discributions
                                                            transition_pseudocount=transition_pseudocount,
                                                            use_pseudocount=use_pseudocount,
                                                            # inertia
                                                            n_jobs=n_jobs,
                                                            ...
                                                            weights=weights_array
                                                            # labels
                                                            # weights  An array of weights, one for each sequence to train on. If None, all sequences are equally weighted
                                                            )
...
def train_model_batched(allele_name, per_kfold_per_length_data, target_lengths, name_prefix="",
                per_kfold_per_length_test_data=None,
                per_kfold_per_length_weights_for_data=None,
                prepared_models=None,
                num_states=12, verbose=False, multiple_check_input=True, n_jobs=1, algorithm='baum-welch',
                lr_decay=0.0,
                minibatch_training=False,
                batches_per_epoch=None,
                batch_size=1, min_iterations=30, max_iterations=1e8,
    ...
    for split_num, per_length_data in per_kfold_per_length_data.items():
        ...
        # list_of_arrays = [per_length_data[target_length] for target_length in target_lengths]
        # list_of_arrays_for_weights = [np.array(per_length_current_weights[target_length]) for target_length in
        #                               target_lengths]
        # list_of_test_arrays = [per_length_test_data[target_length] for target_length in target_lengths]
        # binders_array = np.concatenate(list_of_arrays)
        # weights_array = np.concatenate(list_of_arrays_for_weights)
        #
        # binders_test_array = np.concatenate(list_of_test_arrays)
        ...
        for i in range(min_iterations):
            ...
            print("data shuffled")
            sample_X_test = np.array([[char for char in item] for item in binders_test_array], dtype=object)
            sample_X = np.array([np.array([char for char in item]) for item in binders_array], dtype=object)
            sequence_test_generator = SequenceGenerator(sample_X_test) if use_test else None

            #sequence_test_generator = sample_X_test


           
            current_sequence_generator = BatchedDataGenerator(sample_X, batches_per_epoch=batches_per_epoch,
                                                      batch_size=batch_size)
            total_batches = int(len(current_sequence_generator) / current_sequence_generator.batch_size)
            if init_batch_size is None:
                if batches_per_epoch is None:
                    init_batch_size = total_batches
                else:
                    assert batches_per_epoch < total_batches, f"Batches per epoch should be {total_batches} or lower for given batch size of {batch_size}"
                    init_batch_size = batches_per_epoch
            print(f"Mini_batch: Train Length {len(current_sequence_generator)}, "
                  f"batch_size {current_sequence_generator.batch_size}, batches_per_epoch {current_sequence_generator.batches_per_epoch}, "
                  f"init_batch_size {init_batch_size}")

            

            for current_batch in current_sequence_generator.batches():
                sg = SequenceGenerator(current_batch[0])
                
                model, history = model.fit(sg,
                                       sequences_test=sequence_test_generator,
                                       stop_threshold=stop_threshold,
                                       return_history=True,
                                       verbose=verbose,
...
def init_distribution(amino_acids=None, 
                      ...
                      cycle_max_components=20,
                      cycle_default_pseudocount=50,
                      cycle =False,
                      position_component=False,
                      prepared_probs=None):
    amino_acids_list = list(sorted(set(amino_acids)))  # IMPORTANT. SHould be set because HMM method will convert to set and there willbe different order (non alphabetic)
    position_variants = [str(i) for i in range(19)]
    if not discrete_distribution:
        ...
        elif not equal_probs:
            d_dict = {key: prob for key, prob in zip(amino_acids_list, np.random.default_rng().uniform(0, 1, size=len(amino_acids_list)))}
        else:
        ...
        if position_component:
            ...
            pos_d_dict = {key: prob for key, prob in zip(position_variants, np.random.default_rng().uniform(0, 1, size=len(position_variants)))}
...
def build_model_based_on_params( model_training_params:ModelTrainingParams, amino_acids='ACDEFGHIKLMNPQRSTVWY',verbose=False,
    ...
    ## 0.5 and 0.5 situation both ends
    #     START_WEIGHT = 5
    #     TO_FIRST_CYCLE_WEIGHT = 5
    #     TO_LAST_CYCLE_WEIGHT = 5
    ...
    #     IN_FIRST_CYCLE_WEIGHT = TO_FIRST_JOINER_WEIGHT
    #     IN_LAST_CYCLE_WEIGHT = 1
    #     END_WEIGHT = 5
    #     FROM_LAST_CYCLE_WEIGHT = 1
...
def generate_radom_paths_for_model_mark_core(model, n, per_model_important_states,edge_value="log_probability"):
    ...
    for i in range(n):
        ...
        if np.isnan(start_core) or np.isnan(end_core):
            simplified_path = np.array([probs_dif if node.name in per_model_important_states[model.name] else 0 for node in path_nodes[1:-1]]).astype(np.float64)
        else:
            simplified_path = np.array([probs_dif if node.name in per_model_important_states[model.name] else 0 for node in path_nodes[1:-1]]).astype(np.float64)
...
def get_all_viterbi_paths_for_model_and_sequences_mark_core(model, peptides, per_model_important_states,
                                                            ...
                                                            generate_count=50000, fill_whole_core=False, cumulative_metric=False,
                                                            ):
    """
    ...
    for path_code, path_nodes in tqdm(path_code_to_path.items()):
            ...
            if edge_value == "log_probability":
                ...
                if mark_core:
                    ...
                    if node_to.distribution:
                        cummulative_prob = cummulative_prob + log_probability + np.log(node_to.distribution.parameters[0][peptide[i]])
        ...
        if mark_core:
            ...
            if np.isnan(start_core) or np.isnan(end_core):
                simplified_path = np.array([probs_dif if node.name in per_model_important_states[model.name] else 0 for node in cur_path]).astype(np.float64)
                ...
                if fill_whole_core:
                    ...
                    simplified_path[start_core:end_core] = [probs_dif if cur_path[i].name not in per_model_skip_states[model.name] else 0 for i in range(start_core,end_core)]
...
def calculate_model_core_guesses(allele_peptides,models_dict,per_model_important_states, per_model_skip_states=None, selected_peptides=None):
    ...
    for i, (model_key, model) in enumerate(models_dict.items()):
        ...
        total_df[f'core_start_{i}'] =  viterbi_paths['simplified_path'].apply(lambda x: np.where(np.array(x) > 0)[0]).apply(lambda x: x[0] if len(x) > 0 else 0)
        total_df[f'core_end_{i}'] =  viterbi_paths['simplified_path'].apply(lambda x: np.where(np.array(x) > 0)[0]).apply(lambda x: x[-1] if len(x)> 0 else 1)
        ...
        if selected_peptides:
            for selected_peptide in selected_peptides:
                selected_peptide_to_paths[selected_peptide].append(viterbi_paths.loc[selected_peptide]['simplified_path'])
...
def get_all_viterbi_paths_for_model_and_sequences_preserve_peptides_check_importance_masks(model, peptides, per_model_important_states, edge_value="log_probability", precision=4):
    print(f"Model {model.name}")
    model_graph = model.graph
    ...
    for path in tqdm(unique_paths):
        ...
        result_lens.append(len(path) - 2)
        result_probs.append(prob_for_path)
        result_simplified_paths.append(np.array([1 if node.name in per_model_important_states[model.name] else 0 for node in saved_path_nodes[1:]]))
        # normalize
        result_normalized_probs.append(prob_for_path / len(path))

    paths_df = pd.DataFrame(
        {"path": result_paths, "simplified_path": result_simplified_paths ,"prob": result_probs, "normalized_prob": result_normalized_probs, "len": result_lens,
         "mean_viterbi": mean_viterbies, "peptide_cnts": cnts})
    paths_df = paths_df.sort_values('normalized_prob', ascending=False)
    paths_df = paths_df.reset_index(drop=True)
    return paths_df

def get_all_viterbi_paths_for_model_and_sequences_preserve_peptides(model, peptides, edge_value="log_probability", precision=4):
    print(f"Model {model.name}")
    model_graph = model.graph
...
def get_all_scores_and_sort_by_best(per_allele_per_split_per_length_data, per_name_models):
    ...
    for allele_name in ALLELES:
        ...
        for i, model in enumerate(per_name_models.values()):
            dict_of_result_predictions[f'model_{i}'] = [model.log_probability(peptide) / len(peptide) for peptide in df.peptide]
...
def check_model_alignment(current_model, allele_data_df, allele_name):
    ...
    ww_logo = logomaker.Logo(info_matrix, font_name='Liberation Sans Narrow', flip_below=True,
                             ax=axes)
...
def check_models_integrity_and_shift_params(per_split_models, allele_data_df, experiment_params):
    ...
    for split_num in per_split_models.keys():
        ...
        if max_core_start_pos < left_part:
            ...
            new_core_count_df = count_df.iloc[better_pos_numerical_index:current_core_length + better_pos_numerical_index, :].copy()
        ...
        if new_core_count_df is not None:
            ...
            new_prob_matrix = logomaker.transform_matrix(new_core_count_df,
                                                     ...
                                                     pseudocount = 1,
                                                     background=get_frequencies(new_core_count_df.shape[0]).set_index(new_core_count_df.index), )
...
def train_multiple_models(prepared_models,
    ...
    for allele_name in ALLELES:
        ...
        for run_index in prepared_models[allele_name].keys():
            ...
            per_kfold_per_length_weights = train_data_weigts[allele_name]
            ...
            if model_training_params.check_model_integrity:
                models_to_be_updated = check_models_integrity_and_shift_params(models, per_allele_data_df[allele_name], experiment_params=experiment_params)
...
def reorder_models_by_score_and_flatten_to_by_name_list(histories, models, experiment_params:ExperimentParams, subfolder_to_safe_result : str ):
    path_to_save_models = f"{experiment_params.experiment_result_data_path}/{subfolder_to_safe_result}/"
...
def get_median_scores_for_top_models(per_name_models, allele_data_df, models_fraction=0.7):
    ...
    for model_index in range(number_of_models_to_estimate):
        ...
        scores[:, model_index] = [selected_model.log_probability(peptide) / len(peptide) for peptide in allele_data_df.peptide]
...
def get_data_split_according_to_strategy(
    ...
    if data_split_strategy == "best_model":
        ...
        for allele_name in ALLELES:
            df = per_allele_data_df[allele_name]
            df['score'] = get_median_scores_for_top_models(per_name_models=per_name_models, allele_data_df=df, models_fraction=model_training_params.split_decision_models_fraction)
            per_allele_data_df[allele_name] = df
...
def reassign_data_to_splitted_models_according_to_strategy(per_name_models1, per_name_models2, per_allele_data_df, experiment_params : ExperimentParams):
    model_training_params: ModelTrainingParams = experiment_params.model_training_params
    data_split_strategy = model_training_params.data_split_strategy
    ALLELES = list(per_allele_data_df.keys())
    TOTAL_SPLITS = per_allele_data_df[ALLELES[0]].split.unique().astype(list)
    ...
    if data_split_strategy == "best_model":
        ...
        for allele_name in ALLELES:
            df = per_allele_data_df[allele_name]
            df['score1'] = get_median_scores_for_top_models(per_name_models=per_name_models1, allele_data_df=df, models_fraction=model_training_params.enrichment_split_decision_models_fraction)
            df['score2'] = get_median_scores_for_top_models(per_name_models=per_name_models2, allele_data_df=df, models_fraction=model_training_params.enrichment_split_decision_models_fraction)
            per_allele_data_df[allele_name] = df
    elif data_split_strategy == "single_average_model":
        for allele_name in ALLELES:
            df = per_allele_data_df[allele_name]
...
def get_information_content_estimation_for_models(per_name_models, models_fraction):
    ...
    model_trained_allele_to_info_vector = {allele_name: logomaker.transform_matrix(prob_matrix,
                                                                                   from_type='probability',
                                                                                   to_type='information',
                                                                                   background=get_frequencies(prob_matrix.shape[0])).sum(
        axis=1).values for allele_name, prob_matrix in model_allele_to_matrix.items()}

    allele_to_info_value = {allele_name : sum(item) / len(item) for allele_name, item in model_trained_allele_to_info_vector.items()}
    print("info calculated ", allele_to_info_value)
    model_name_to_info_value = {model_name : allele_to_info_value[model_name_to_allele[model_name]] for model_name in model_names}
...
def get_model_distances(per_name_models1, per_name_models2, models_fraction):
    ...
    model_name_to_distance1 = {model_name : allele_to_model_distance[model_name_to_allele1[model_name]] for model_name in model_names1}
    model_name_to_distance2 = {model_name : allele_to_model_distance[model_name_to_allele2[model_name]] for model_name in model_names2}
...
def make_single_split_and_enrich(per_name_models,
   ...
   # print("length of the data before split", len(per_allele_data_df[list(per_allele_data_df.keys())[0]]))
    ...
    per_allele_per_run_per_split_new_histories1 = train_multiple_models(
        ...
        test_data = test_data,
        train_data_weigts=train_data_weights1,
        experiment_params=experiment_params,
    ...
    per_allele_per_run_per_split_new_histories2 = train_multiple_models(
        ...
        test_data = test_data,
        train_data_weigts=train_data_weights2,
        experiment_params=experiment_params,
    ...
    allele_to_distance = {allele_name : distance / (2*(allele_to_info_value1[allele_name] + allele_to_info_value2[allele_name])) for allele_name, distance in allele_to_distance.items()}
    ...
    name_to_distance1 = {model_name : distance /(2*(allele_to_info_value1[model_name_to_allele1[model_name]] + allele_to_info_value2[model_name_to_allele1[model_name]])) for model_name, distance in name_to_distance1.items()}
    name_to_distance2 = {model_name : distance /(2*(allele_to_info_value1[model_name_to_allele2[model_name]] + allele_to_info_value2[model_name_to_allele2[model_name]])) for model_name, distance in name_to_distance2.items()}
    ...
    passed_distance_cutoff1 = {key for key, distance_value in name_to_distance1.items() if distance_value > distance_threshold}
    passed_distance_cutoff2 = {key for key, distance_value in name_to_distance2.items() if distance_value > distance_threshold}
    alleles_passed_distance_cutoff = {key for key, similarity_value in allele_to_distance.items() if similarity_value > distance_threshold}
    ...
    for allele_name in allele_to_info_matrix1.keys():
        ...
        data_size1 = sum(len(peptides) for kfold, per_length_data in train_data1[allele_name].items() for cur_length , peptides in per_length_data.items() )
        data_size2 = sum(len(peptides) for kfold, per_length_data in train_data2[allele_name].items() for cur_length , peptides in per_length_data.items() )


        plot_info_matrices_to_file(allele_to_info_matrix1[allele_name],allele_to_info_matrix2[allele_name],
                                   folder_to_save=path_to_save_pictures,
                                   main_title=f"Allele {allele_name} for model {iteration_name} \n \
                                   info ({np.round(info_value1,2)}/{np.round(info_value2,2)}) passed ({passed1}/{passed2}),\n \
                                   distance {allele_to_distance[allele_name]:.2f} \ final split was ( {data_size1} \ {data_size2})",
...
def hierarchically_train_splited_models(per_name_models, train_data, test_data, additional_data, experiment_params:ExperimentParams, total_layers_to_do=3):
    ...
    with open('out.txt', 'w') as f:
        ...
        while iteration_deque:
            ...
            if len(finalized_models) > 0:
                ...
                result_models[iteration] = {name: model for name, model in per_name_current_models.items() if name in finalized_models}
            ...
            if (len(iteration_deque) == 0) or (len(iteration) < len(iteration_deque[0])):  # we finished the whole layer
                ...
                rest , fig = plot_split_diagnostics(per_allele_data_df=per_allele_scores_data, ann_df=additional_data)
                ...
                for allele_name in per_allele_total_data.keys():
                    ...
                    for model_num, (iteration_num, current_result_models_dict) in enumerate(result_models.items()):
                        ...
                        total_data[f'score_general_{analysis_models_observed+model_num}'] = median_score_for_current_models

                    X_matrix = total_data[[f'score_general_{i}' for i in range(analysis_models_observed+resul_models_num)]].copy()
