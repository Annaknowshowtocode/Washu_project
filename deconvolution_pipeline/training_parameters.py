...
DATA_DIR     = Path(os.environ.get("HMM_DATA_DIR",     PROJECT_ROOT / "data"))
MAT_DIR      = Path(os.environ.get("HMM_MAT_DIR",      DATA_DIR / "reference_matrices"))
PEPTIDES_DIR = Path(os.environ.get("HMM_PEPTIDES_DIR", DATA_DIR / "peptides"))
RESULTS_DIR  = Path(os.environ.get("HMM_RESULTS_DIR",  PROJECT_ROOT / "results"))

for _d in (DATA_DIR, MAT_DIR, PEPTIDES_DIR, RESULTS_DIR):
...
class DataScenarioParams(param.Parameterized):
    data_scenario = param.Selector(objects=["simulated",
                                            ...
                                            "MixMHCpred",
                                            "preprocessed_IEDB"])
    data_path_base = param.Foldername(str(DATA_DIR), check_exists=False)
...
class ModelTrainingParams(param.Parameterized):
    ...
    anchor_top_aas = param.Integer(default=5, bounds=(1, 20)) #сколько аминокислот главные в якорных позициях
    ...
    # Splits
    trash_model_information_threshold = param.Number(default=0.35) #0.35
...
class ExperimentParams(param.Parameterized):
    ...
    def _update_experiment_path(self):
        ...
        self.experiment_result_data_path = str(RESULTS_DIR / self.experiment_name)